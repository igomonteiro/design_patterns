...
test('Deve criar um chat entre participantes', () => {
  ...
  expect(participantb.messages[0]).toBe(
    'Participante B recebeu a mensagem Hello do participante: A'
  );
  expect(participantc.messages[0]).toBe(
    'Participante C recebeu a mensagem Hello do participante: A'
  );
});
...
test('Deve manda uma mensagem de um participante para outro', () => {
  const participanta = new Participant('A');
  const participantb = new Participant('B');
  const participantc = new Participant('C');
  const channel = new Channel();
